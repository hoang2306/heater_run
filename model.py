...
class Heater:
    ...
    def build_model(self):
        # Sử dụng tf.Variable cho lr_placeholder
        self.lr_placeholder = tf.Variable(0.001, dtype=tf.float32, name='learn_rate')

        # Sử dụng tf.keras.Input cho các giá trị kiểu placeholder
        self.is_training = tf.keras.Input(shape=(), dtype=tf.bool, name='is_training')
        self.target = tf.keras.Input(shape=[None], dtype=tf.float32, name='target')

        self.Uin = tf.keras.Input(shape=[None, self.rank_in], dtype=tf.float32, name='U_in_raw')
        self.Vin = tf.keras.Input(shape=[None, self.rank_in], dtype=tf.float32, name='V_in_raw')

        dim = self.dim
        self.reg_loss = 0.

        if self.phi_v_dim > 0:
            self.Vcontent = tf.keras.Input(shape=[None, self.phi_v_dim], dtype=tf.float32, name='V_content')
            self.dropout_item_indicator = tf.keras.Input(shape=[None, 1], dtype=tf.float32, name='dropout_item_indicator')

            vcontent_gate, vcontent_gate_reg = dense_fc(self.Vcontent, dim, 'vcontent_gate_layer')
            vcontent_gate = tf.nn.tanh(vcontent_gate)
            ...
            vcontent_expert_concat = tf.concat(vcontent_expert_list, 1)

            vcontent_expert_concat = tf.linalg.matmul(tf.reshape(vcontent_gate, [-1, 1, dim]), vcontent_expert_concat)
            Vcontent_last = tf.reshape(tf.nn.tanh(vcontent_expert_concat), [-1, self.rank_out])

            self.Vin_filter = 1 - self.dropout_item_indicator

            diff_item_loss = self.alpha * (tf.reduce_sum(tf.reduce_sum(tf.square(Vcontent_last - self.Vin),
                                                                    axis=1, keepdims=True)))
            v_last = (self.Vin * self.Vin_filter + Vcontent_last * (1 - self.Vin_filter))
        else:
            v_last = self.Vin
        ...
        if self.phi_u_dim > 0:
            self.Ucontent = tf.keras.Input(shape=[None, self.phi_u_dim], dtype=tf.float32, name='U_content')
            self.dropout_user_indicator = tf.keras.Input(shape=[None, 1], dtype=tf.float32, name='dropout_user_indicator')

            ucontent_gate, ucontent_gate_reg = dense_fc(self.Ucontent, dim, 'ucontent_gate_layer')
            ucontent_gate = tf.nn.tanh(ucontent_gate)
            ...
            ucontent_expert_concat = tf.linalg.matmul(tf.reshape(ucontent_gate, [-1, 1, dim]), ucontent_expert_concat)
            Ucontent_last = tf.reshape(tf.nn.tanh(ucontent_expert_concat), [-1, self.rank_out])

            self.Uin_filter = 1 - self.dropout_user_indicator

            diff_user_loss = self.alpha * (tf.reduce_sum(tf.reduce_sum(tf.square(Ucontent_last - self.Uin),
                                                                    axis=1, keepdims=True)))
            u_last = (self.Uin * self.Uin_filter + Ucontent_last * (1 - self.Uin_filter))
        else:
            u_last = self.Uin
